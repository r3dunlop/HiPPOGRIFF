...
int TMidasOnline::eventRequest(const char* bufferName, int eventId, int triggerMask, int samplingType, bool poll)
{
  ...
  status = bm_set_cache_size(r->fBufferHandle, 1000000, 0);

	//status = bm_set_cache_size(r->fBufferHandle, 	 8388608, 0);
	//status = bm_set_cache_size(r->fBufferHandle, 	 16777216, 0);

	printf("rpc_is_remote()\t= %i\n",rpc_is_remote());
	printf("status         \t= %i\n",status);
...
unsigned char TMidasOnline::odbReadByte(const char*name,int index,unsigned char defaultValue)
{
  unsigned char value = defaultValue;
  if (odbReadAny(name,index,TID_BYTE,&value) == 0)
    return value;
  else
    return defaultValue;
};
...
{
  static std::map<std::string, HNDLE> parentMap;
  ...
  std::string parentString = name_str.substr(0,name_str.find_last_of('/'));  
  std::string childString  = name_str.substr(name_str.find_last_of('/')+1,name_str.length());

  if (parentMap.find(parentString) != parentMap.end() ) {
    hdir = parentMap[parentString];
  } else {
    status = db_find_key( fDB, 0, (char*)parentString.c_str(), &hdir);
    if (status != SUCCESS) {
      cm_msg(MERROR, "TMidasOnline", "Cannot read parent \'%s\' from odb, status %d", name, status);
      return -1;
    }
    printf ("Found parent %s.\n", parentString.c_str());
    parentMap[parentString] = hdir;
  }

  if (size == 0)
    size = rpc_tid_size(tid);

  status = db_find_key (fDB, hdir, (char*)childString.c_str(), &hkey);
  if (status == SUCCESS)
    {
