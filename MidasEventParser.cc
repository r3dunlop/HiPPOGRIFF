	...
	for(unsigned int wordcount = 0; wordcount<mSize; wordcount++){
		switch(((*(dataword+wordcount) & PacketMask)  >> PacketShift)){
		  case(kHeader):
				...
				SetNFilterPatterns(*(dataword+wordcount));
				SetDataType(*(dataword+wordcount));
				SetPileupType(*(dataword+wordcount));
				SetAddress(*(dataword+wordcount));
				SetDetectorType(*(dataword+wordcount));
				//Move to the next word where PPG should be stored
				wordcount++;
				SetPPGPattern(*(dataword+wordcount));
				//Move to the next word where Master Filter ID should be stored
				wordcount++;
				SetMasterFilterID(*(dataword+wordcount));
				//Move to the next word where Master Filter Pattern should be stored
				wordcount++;
				SetMasterFilterPattern(*(dataword+wordcount));
		  ...
		  case(kChannelTrigger):
				SetChannelTriggerID(*(dataword+wordcount));
		  ...
		  case(kLowerTimestamp):
				SetLowerTimestamp(*(dataword+wordcount));
				...
				break;
		  case(kWaveform):
				SetWaveForm(*(dataword+wordcount));
				break;
		  case(kDeadtime):
				...
				//possible in the griff frag to go into the bad tree
					SetDeadTime(*(dataword+wordcount));
					SetUpperTimestamp(*(dataword+wordcount));
					deadtime_flag = true;
					...
					while(nHits < GetPileupType()){// && bank.GotData()){// && IsGoodFrag()){
						...
						SetHighKBits(*(dataword+wordcount));
						SetPulseHeight(*(dataword+wordcount));
						wordcount++;
						//Move Forward to the next word where the lower K-value and CFD Timing should be stored
						SetLowKBits(*(dataword+wordcount));
						SetCFDTiming(*(dataword+wordcount));
						nHits++;
						GrifFrag.hitNumber++;
			...
			//	}
				break;
		  case(kTrailer):
				CheckTrailerWord(*(dataword+wordcount));
				trailer_flag = true;
...
void MidasEventParser::SetWaveForm(uint32_t& dataword){
...
void MidasEventParser::SetNFilterPatterns(uint32_t& dataword){
...
void MidasEventParser::SetDataType(uint32_t& dataword){
...
void MidasEventParser::SetAddress(uint32_t& dataword){
...
void MidasEventParser::SetDetectorType(uint32_t& dataword){
...
void MidasEventParser::SetPileupType(uint32_t& dataword){
...
void MidasEventParser::SetPPGPattern(uint32_t& dataword){
...
void MidasEventParser::SetMasterFilterID(uint32_t& dataword){
...
void MidasEventParser::SetMasterFilterPattern(uint32_t& dataword){
...
void MidasEventParser::SetChannelTriggerID(uint32_t& dataword){
...
void MidasEventParser::SetLowerTimestamp(uint32_t& dataword){
...
void MidasEventParser::SetUpperTimestamp(uint32_t& dataword){
...
void MidasEventParser::SetLowKBits(uint32_t& dataword){
...
void MidasEventParser::SetPulseHeight(uint32_t& dataword){
...
void MidasEventParser::SetCFDTiming(uint32_t& dataword){

	CFDTiming.push_back((dataword & CFDTimingMask) >> CFDTimingShift);
	hCFD->Fill((dataword & CFDTimingMask) >> CFDTimingShift);
}

void MidasEventParser::CheckTrailerWord(uint32_t& dataword){
